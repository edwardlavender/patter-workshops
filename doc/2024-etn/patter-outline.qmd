...
Lavender, E. et al. (2024). Particle algorithms for animal movement modelling in autonomous receiver networks. bioRxiv 2024.09.16.613223. <https://doi.org/10.1101/2024.09.16.613223>

-   **For the package**, see:

Lavender, E. et al. (2024). `patter`: particle algorithms for animal tracking in R and Julia. bioRxiv 2024.07.30.605733. <https://doi.org/10.1101/2024.07.30.605733>
...
op <- options(patter.verbose = TRUE)
```
...
- The particle filter approximates the partial marginal distribution of the individual's location given the data _up to and including_ each time step, that is, $f(\mathbf{s}_t \mid \mathbf{y}_{1:t})$
- If we run the particle filter forwards and backwards, we can use the two-filter smoother to account for _all_ data, that is, $f(\mathbf{s}_t \mid \mathbf{y}_{1:T})$
...
```{r, echo = FALSE}
map_pou(.map = map, .coord = smo$states, .verbose = FALSE)$ud |> invisible()
```
