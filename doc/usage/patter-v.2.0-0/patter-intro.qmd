...
-   There are many packages for animal tracking (Joo et al., [2020](https://doi.org/10.1111/1365-2656.13116))
...
-   `patter` focuses specifically on **inference** for SSMs using **particle algorithms**
...
Lavender, E. et al. ([2025a](https://doi.org/10.1111/2041-210X.70028)). Particle algorithms for animal movement modelling in receiver arrays. Meth. Ecol. Evol. <https://doi.org/10.1111/2041-210X.70028>

-   **For the package**, see the package paper:

Lavender, E. et al. ([2025b](https://doi.org/10.1111/2041-210X.70029)). `patter`: particle algorithms for animal tracking in R and Julia. Meth. Ecol. Evol. <https://doi.org/10.1111/2041-210X.70029>
...
Lavender, E. et al. ([2025a](https://doi.org/10.1111/2041-210X.70028)). Supporting Information.
...
3. Inference for the latent states is performed using particle filtering and smoothing
...
![`patter` produces refined maps of space use (Lavender et al., [2025a](https://doi.org/10.1111/2041-210X.70028))](../../fig/patter-maps.png)

## Why should I use `patter`? 

![`patter` consistently outperforms heuristic methods (Lavender et al., [2025a](https://doi.org/10.1111/2041-210X.70028))](../../fig/patter-me.png)

## Why should I use `patter`?

![`patter` produces refined estimates of residency (Lavender et al., [2025b](https://doi.org/10.1111/2041-210X.70029))](../../fig/patter-residency.png)
...
::: callout-important
A recent `Julia` version (≥ 1.10 [LTS]) is recommended. Download links are [here](https://julialang.org/downloads/) or you can use [`juliaup`](https://github.com/JuliaLang/juliaup). Follow the [README](https://github.com/edwardlavender/patter) instructions for further guidance. 
:::
...
## Project set up: `JULIA` options {.smaller .scrollable}
...
## Set movement model (3/7) {.smaller}
...
## Run particle filter (5/7) {.smaller}
...
-   Where parameters are unknown, you can:
    1.  See Lavender et al. ([2025a](https://doi.org/10.1111/2041-210X.70028)) for guidance on parameter sensitivity
...
-   `ModelObsContainer`s are effectively a redundant use of the data that mitigates particle degeneracy
...
-   At $t = 1$:
    ...
    1.  Sample initial particles (locations → `State`s), via `Patter.simulate_states_init()`
...
::: callout-note
The default `sigma` in `map_dens()` differs from `density.ppp()`. `map_dens()` uses `bw.h()`, which sets the bandwidth based on the combined variance of summarised coordinates using the ad-hoc method (Worton, [1989](https://doi.org/10.2307/1938423)); i.e., `bw.h = function(X) sqrt(0.5 * (var(X$x) + var(X$y))) * (X$n^(-1/6))`. 
:::
...
-   To incorporate custom structures, we need to:
    ...
    4.  Write any R wrappers for the routines
...
## Custom `State` and `ModelMove` {.smaller}

-   For custom `State` and `ModelMove` structures, we need to:
    1.  Register the structures in `Julia`
    2.  (optional) Write a `Patter.states_init()` methods to translate initial coordinates to `State`s
...
-   For custom `ModelObs` structures, we need to:
    ...
    1.  (optional) Write `Patter.map_init()` method to sample initial `State`s efficiently
...
## Custom workflow {.smaller .scrollable}
...
::: callout-note
See the [`patter-eval`](https://github.com/edwardlavender/patter-eval/blob/main/R/008-run-patter.R) project for an example where socket clusters were used.
:::
...
3.  Backwards sampling algorithms 
...
5.  Other model-based inference packages (e.g., [Wahoo.jl](https://github.com/scheidan/Wahoo.jl))
